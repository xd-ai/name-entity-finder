import unittest
from src.ner_client import NamedEntityClient
...
class TestNerClient(unittest.TestCase):
    ...
    def test_get_ents_given_spacy_PERSON_is_returned_serializes_to_Person(self):
        ...
        model.returns_doc_ent(doc_ents)
        ner = NamedEntityClient(model)
        result = ner.get_ents('...')
    ...
    def test_get_ents_given_multiple_ents_serializes_all(self):
        model = NerModelTestDouble('eng')
        doc_ents = [{'text': 'Africa', 'label_': 'GPE'}, {'text': 'Jean-Pierre Polnareff', 'label_': 'PERSON'}]
        model.returns_doc_ent(doc_ents)
        ner = NamedEntityClient(model)
        result = ner.get_ents('...')
        expected_result = {
                'ents': [
                    {'ent': 'Africa', 'label': 'Location'}, 
                    {'ent': 'Jean-Pierre Polnareff', 'label': 'Person'}], 
                'html': ''
                }
        self.assertListEqual(result['ents'], expected_result['ents'])
